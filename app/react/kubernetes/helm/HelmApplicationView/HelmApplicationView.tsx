...
export function HelmApplicationView() {
  ...
  return (
    <>
      <PageHeader
        title={t('kubernetes.helm.release.title')}
        breadcrumbs={[
          { label: t('kubernetes.helm.common.applications'), link: 'kubernetes.applications' },
          name,
        ]}
        reload
      />
...
function HelmDetails({
  ...
  if (isError) {
    return (
      <Alert color="error" title={t('kubernetes.helm.release.errors.loadDetails')} />
    );
  }

  if (!release || !selectedRevision) {
    return <Alert color="error" title={t('kubernetes.helm.release.errors.noDetails')} />;
  }
