...
export function CreateContainerInstanceForm({
  ...
  async function onSubmit(values: ContainerInstanceFormValues) {
    try {
      ...
      notifications.success(t('azure.containerInstances.notifications.created'), values.name);
      router.stateService.go('azure.containerinstances');
    } catch (e) {
      notifications.error(t('common.failure'), e as Error, t('azure.containerInstances.notifications.createFailure'));
    }
  }
}
