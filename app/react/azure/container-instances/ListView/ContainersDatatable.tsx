...
export function ContainersDatatable({ dataset, onRemoveClick }: Props) {
  ...
  return (
    <Datatable
      ...
      title={t('azure.containerInstances.containers')}
      titleIcon={Box}
        ...
        <div className="flex gap-2">
          <Authorized authorizations="AzureContainerGroupDelete">
            <DeleteButton
              ...
              confirmMessage={t('azure.containerInstances.confirmDeleteSelected')}
            />
          </Authorized>

          <Authorized authorizations="AzureContainerGroupCreate">
            <AddButton data-cy="add-container-button">{t('azure.containerInstances.addContainer')}</AddButton>
          </Authorized>
        </div>
      )}
    />
