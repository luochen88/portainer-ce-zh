...
export function TableActions({
  ...
  return (
    <div className="flex items-center gap-2">
      <DeleteButton
        confirmMessage={t('edge.groups.confirmRemoveSelected')}
        disabled={selectedItems.length === 0}
        onConfirmed={() => handleRemove(selectedItems)}
        data-cy="remove-edge-group-button"
      />

      <AddButton data-cy="add-edge-group-button">{t('edge.groups.add')}</AddButton>
    </div>
  );
  ...
  async function handleRemove(selectedItems: Array<EdgeGroup>) {
    ...
    removeMutation.mutate(ids, {
      onSuccess: () => {
        notifySuccess(t('common.success'), t('edge.groups.notifications.removed'));
      },
    });
  }
}
